...
BEGIN
  ...
  UPDATE chess.messages set request_id = v_request_id, send_time = NOW() WHERE message_id in
                                                                               (SELECT message_id FROM message_ids_tmp);

  DROP TABLE IF EXISTS message_ids_tmp;

  RETURN QUERY SELECT chess.messages.user_id, chess.messages.data FROM chess.messages WHERE chess.messages.request_id = v_request_id;
END;
